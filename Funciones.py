...
def extraccion_audio():
    ...
    nombre_clip = input(
        "Ingrese el nombre del archivo completo(video, junto con la extensión de este): "
    )
...
def conversor_video_a_audio():
    ...
    audio = video.audio
    nombre_arhivo = input(
        "Ingrese como quiere escribir el audio y la extensión(Ej: Audio.wav): "
    )
...
def audio_a_texto():
    ...
    r = sr.Recognizer()
    print(
        "Recuerda que tienes que tener los documentos con el mismo nombre principal y distinto número"
    )
    nombre_documento = input(
        "Ingresa el nombre del documento y extension. Ej: Clip1.wav: "
    )
...
def division_audio_partes_iguales():
    ...
    print(
        f"Puedes dividir el video en {my_audio_1.duration / 180} partes de 3 minutos cada una"
    )
...
def peticion_2():
    ...
    print(
        "1 ) Dividir el audio en partes iguales(de un archivo de audio) y escribirlos como archivos separados"
    )
